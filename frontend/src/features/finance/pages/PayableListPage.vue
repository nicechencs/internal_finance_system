<template>
  <div class="page-container">
    <PageHeader
      v-if="!embedded"
      title="应付管理"
      description="跟踪和管理应付账款"
    >
      <template #primary>
        <el-button v-if="userStore.canEdit" type="primary" @click="handleCreate">新增应付</el-button>
      </template>
    </PageHeader>
    ...
    <el-row :gutter="isMobile ? 12 : 24" class="stat-cards" v-if="!embedded && !externalFilters">
      <el-col :xs="12" :sm="12" :md="6">
        <StatCard
          ...
          label="应付总数"
          :count="`${statistics.pendingCount} 待付`"
          theme="info"
        />
      </el-col>
      <el-col :xs="12" :sm="12" :md="6">
    ...
    <div class="table-section">
      <ResponsiveList :items="payables" :loading="loading">
        <template #table>
      <el-table :data="payables" v-loading="loading" class="resizable-table" border allow-drag-last-column @header-dragend="handleHeaderDragend" @sort-change="handleSortChange">
        <el-table-column prop="counterpartyName" label="对方" :width="getColumnWidth('counterpartyName', TABLE_COLUMN_WIDTH.company)">
          <template #default="{ row }">
            ...
            <template v-else-if="row.customerId">
        ...
        <el-table-column prop="payableTypeName" label="业务类型" :width="getColumnWidth('payableTypeName', TABLE_COLUMN_WIDTH.shortText)">
          <template #default="{ row }">
            <el-tag v-if="row.payableTypeName" size="small" type="info">{{ row.payableTypeName }}</el-tag>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column prop="totalAmount" label="应付金额" sortable="custom" :width="getColumnWidth('totalAmount', TABLE_COLUMN_WIDTH.amount)" align="right">
          <template #default="{ row }">
            {{ formatMoney(row.totalAmount) }}
          </template>
        </el-table-column>
        <el-table-column prop="paidAmount" label="已付金额" :width="getColumnWidth('paidAmount', TABLE_COLUMN_WIDTH.amount)" align="right">
          <template #default="{ row }">
            {{ formatMoney(row.paidAmount) }}
          </template>
        </el-table-column>
        <el-table-column prop="remainingAmount" label="未付金额" sortable="custom" :width="getColumnWidth('remainingAmount', TABLE_COLUMN_WIDTH.amount)" align="right">
          <template #default="{ row }">
        ...
        <template #card="{ item }">
          <MobileListCard
            :title="getCounterpartyLabel(item) || item.projectName || '未命名应付'"
            :amount="item.remainingAmount"
            :amount-type="item.remainingAmount > 0 ? 'expense' : 'neutral'"
            :signed="false"
          ...
          >
            ...
            <template #meta>
              {{ joinMeta(item.projectName, item.payableTypeName, item.dueDate ? formatDateTime(item.dueDate, 'date') : '') }}
            </template>
            <template #footer>
              应付 {{ formatMoney(item.totalAmount) }} · 已付 {{ formatMoney(item.paidAmount) }}
            </template>
          </MobileListCard>
        </template>
        <template #pagination>
...
import { getPayables, deletePayable, getPayableStatistics } from '@/features/finance/api/payable'
...
import type { Payable, PayableStatistics } from '@/features/finance/types/payable'
...
import PayableDetail from '@/features/finance/pages/PayableDetailPage.vue'
import PayableForm from '@/features/finance/components/PayableForm.vue'
...
const { getColumnWidth, getColumnMinWidth, handleHeaderDragend } = useResizableTableColumns('payable-list')
const loading = ref(false)
const payables = ref<Payable[]>([])
const projects = ref<Project[]>([])
const suppliers = ref<Supplier[]>([])
const detailVisible = ref(false)
const currentPayableId = ref<number>(0)
const formVisible = ref(false)
const currentPayable = ref<Payable | null>(null)
...
const buildFilterParams = (): Record<string, any> => {
  ...
  if (filters.tagIds.length > 0) {
    params.tagFilters = [{ scope: 'payable', tagIds: filters.tagIds, matchMode: 'or' }]
  }
  return params
}

const { statistics, loadStatistics } = useListPageStatistics<PayableStatistics>({
  fetchStatistics: getPayableStatistics,
  initialStatistics: {
    totalCount: 0,
    pendingCount: 0,
    partialCount: 0,
...
const loadPayables = async () => {
  ...
  try {
    ...
    if (activeFilters.tagIds && activeFilters.tagIds.length > 0) {
      params.tagFilters = [{ scope: 'payable', tagIds: activeFilters.tagIds, matchMode: 'or' }]
    }

    const { data } = await getPayables(params)
    payables.value = data.data.items
...
const handleManageTags = (row: Payable) => {
  tagEditTarget.value = { id: row.id, name: row.supplierName || row.customerName || row.personName || '应付#' + row.id }
...
const handleDelete = async (row: Payable) => {
  try {
    await ElMessageBox.confirm('确定要删除这条应付记录吗？', '提示', {
...
const handleReset = async () => {
  ...
  if (Object.keys(route.query).length > 0) {
    await router.replace({ name: 'PayableList' })
  }
